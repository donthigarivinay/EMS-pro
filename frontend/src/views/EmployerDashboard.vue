  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="md:col-span-1 flex flex-col justify-center">
        <h2 class="text-2xl font-bold text-white tracking-tight">Leave Requests</h2>
        <p class="text-sm text-slate-500 mt-1">Manage all employee applications</p>
      </div>
      ...
      <div class="md:col-span-2 grid grid-cols-2 gap-4">
        ...
        <div class="glass-panel group hover:bg-white/5 transition-all p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">Total Requests</p>
            <p class="mt-1 text-2xl font-bold text-white">{{ totalLeaves }}</p>
          </div>
        </div>
        <!-- Pending Card -->
        <div class="glass-panel group hover:bg-white/5 transition-all p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
          <div class="h-12 w-12 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-600 flex-shrink-0">
            <Clock class="w-6 h-6" />
          </div>
          <div>
            <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">Pending</p>
            <p class="mt-1 text-2xl font-bold text-amber-600">{{ pendingLeaves }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="reviewLeave !== null" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md backdrop-blur-sm">
        <div class="glass-panel border border-white/10 shadow-2xl shadow-black/50 w-full max-w-lg overflow-hidden flex flex-col ring-1 ring-white/10">
          <div class="px-6 py-4 border-b border-white/10 flex items-center justify-between bg-white/5">
            <h3 class="text-lg font-bold text-white">Review Leave Request</h3>
          ...
          <div class="p-6">
            ...
            <div class="mb-6">
              <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Admin Comment (Optional)</label>
              <textarea v-model="reviewComment" rows="3" placeholder="Add your notes or reasoning here for the employee to see..." class="block w-full rounded-xl border-white/10 shadow-md shadow-black/20 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 bg-black/20 border resize-none outline-none"></textarea>
            </div>
            ...
            <div class="flex gap-3 pt-2 border-t border-white/10">
              ...
              <button @click="submitReview('Approved')" :disabled="isSubmittingReview" class="flex-1 bg-indigo-600 text-white font-semibold py-2.5 px-4 rounded-xl shadow-md shadow-black/20 hover:bg-indigo-700 transition focus:outline-none flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                <span v-else class="flex items-center gap-2"><CheckCircle class="w-4 h-4" /> Approve</span>
              </button>
            </div>
          </div>
        </div>
...
const openUserProfile = async (userId) => {
  if (!userId) return
  selectedUser.value = {} // Placeholder to open modal
