              ...
              <tbody class="bg-white divide-y divide-slate-100">
                <template v-for="(leave, index) in leaves" :key="leave._id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <a v-if="leave.attachmentUrl" :href="'http://localhost:5000' + leave.attachmentUrl" target="_blank" class="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200 hover:bg-indigo-100 transition-colors uppercase tracking-wider">
...
const fetchBalance = async () => {
  try {
    const { data } = await axios.get('http://localhost:5000/api/users/balance')
    balance.value = data
  } catch (err) {
